...
class KNNPlayer:
    def __init__(self, K=1, safe_reward=-1, bomb_reward=1):
        self.K = K
        self.safe_reward = safe_reward
        self.bomb_reward = bomb_reward
        self.np_actions = 0
    ...
    def knn_q(self, config):
        ...
        for conf in self.actions:
            # dist = (sum([(config[i] - conf[i])**2 for i in range(len_conf)]))**.5
            dist = 0
            for i in range(len_conf):
                a = config[i] if config[i] is not oursweeper.TileStatus.COVERED.value else 0
                b = conf[i] if conf[i] is not oursweeper.TileStatus.COVERED.value else 0
            ...
            if len(knn) < self.K:
                knn[dist] = self.actions[conf].Q
            else:
                for d in sorted(knn.keys(), reverse = True):
                    if dist < d:
                        del knn[d]
                        knn[dist] = self.actions[conf].Q
                        break
            ...
            for j in range(len_col):
                ...
                for u in [-1, 0, 1]:
                    for v in [-1, 0, 1]:
                        ...
                        q_val = self.actions[config].Q if config in self.actions else self.knn_q(config)
                        count_val = self.actions[config].count if config in self.actions else 0
