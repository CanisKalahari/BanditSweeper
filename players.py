...
class MegreedyPlayer:
    ...
    def mind_play(self, play_result):
        conf = self.last_call
        reward = self.bomb_reward if play_result.explosion else self.safe_reward
                ...
                for u in [-1, 0, 1]:
                    for v in [-1, 0, 1]:
                        ...
                        if config in self.actions:
                            ...
                            q_val = -(1e10) # This ensures a new action is chosen over already played actions (negative value as to assume its safe)
